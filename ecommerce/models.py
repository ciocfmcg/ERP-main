...
class OrderQtyMap(models.Model):
    ...
    status = models.CharField(choices = ORDERQTYMAP_CHOICES , max_length = 10 , default='created' )
    updated = models.DateTimeField(auto_now=True)
    refundAmount = models.PositiveIntegerField(null = True , default = 0)
...
class Order(models.Model):
    created = models.DateTimeField(auto_now_add = True)
    updated = models.DateTimeField(auto_now=True)
    user =  models.ForeignKey(User, null = False , related_name = 'orders')
    totalAmount = models.PositiveIntegerField(default = 0)
    orderQtyMap = models.ManyToManyField(OrderQtyMap , related_name='order' ,blank=True)
